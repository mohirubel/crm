...
import { User, ChevronDown, LogOut, DollarSign, TrendingUp, TrendingDown, Package, AlertTriangle, ShoppingBag, UserCheck, UserX, Clock, Calendar, Users } from 'lucide-react';
...
const MainDashboard = () => {
  ...
  const absentEmployees = [
    ...
    { name: 'Sarah Johnson', department: 'Marketing', reason: 'Personal Leave' },
  ...
  const recentOrders = [
    { id: 'ORD-001', customer: 'Alice Cooper', amount: 1299, status: 'Processing' },
    ...
    { id: 'ORD-005', customer: 'Eva Green', amount: 1599, status: 'Processing' }
  ];
          ...
          <div className="ml-auto sm:ml-0 mr-2 sm:mr-0">
            <div className="flex items-center space-x-4">
              <span className="text-sm text-gray-600 cursor-pointer flex items-center gap-1">
                <User className="w-4 h-4" /> Admin User
              </span>
              <span className="text-gray-300">|</span>
              <button className="text-sm text-gray-600 cursor-pointer flex items-center gap-1">
            ...
            <div className="space-y-2">
              {recentOrders.slice(0, 5).map((order, index) => (
                <div key={index} className="flex flex-col text-xs">
                  ...
                  <span className={`text-xs px-2 py-1 w-fit rounded-md border ${getStatusBadgeColor(order.status)}`}>
                    {order.status}
                  </span>
                </div>
              ))}
          ...
          <CardContent>
            <div className="space-y-3">
              ...
              <div>
                ...
                <p className="text-lg font-bold text-green-600">${monthlySales.toLocaleString()}</p>
              </div>
